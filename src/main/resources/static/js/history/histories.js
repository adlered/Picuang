                                ...
                                axios.get('/api/day?year=' + year + '&month=' + month)
                                    .then(function (dayRes) {
                                        $.each(dayRes.data, function (key, data) {
                                            var day = data;
                                            $("#" + year + "-" + month + "-p").append("<h4>" + day + "日</h4>");
                                            axios.get('/api/list?year=' + year + '&month=' + month + '&day=' + day)
                                                .then(function (listRes) {
                                                    $.each(listRes.data, function (key, data) {
                                                        var list = data;
                                                        $("#" + year + "-" + month + "-p").append("<a href='" + data.path + "' target='_blank'><img src='" + data.path + "' style='width: auto; height: auto; max-width: 128px; max-height: 128px; margin: 8px 4px' class='img-thumbnail'></a>");
                                                        if (i == 0) {
                                                            $("#histories").prepend("" +
